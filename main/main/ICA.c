...
void ICA(unsigned char origin[][256], struct pca pcaTemp, double ny[][1024], double nw[][64], double navg[1024], int e, double f){
	FILE* fp1, * fp2, * fp3, * fp4, * fp5; // fp1:pca基底, fp2:ica基底, fp3:ica係数, fp4:
	...
	/*　ファイルオープン　*/
	if ((fp1 = fopen("OUTPUT\\PCA_BASIS.pgm", "wb")) == NULL){  //pgm > グレースケール画像を扱うための拡張子
		fprintf(stderr, "Can not open file\n");
	}

	if ((fp2 = fopen("OUTPUT\\ICA_test.pgm", "wb")) == NULL){
		fprintf(stderr, "Can not open file\n");
	}

	if ((fp3 = fopen("OUTPUT\\y.csv", "w")) == NULL){
		fprintf(stderr, "Can not open file\n");
	}

	if ((fp4 = fopen("OUTPUT\\ICA_SAI.pgm", "wb")) == NULL){
		fprintf(stderr, "Can not open file\n");
	}

	if ((fp5 = fopen("OUTPUT\\ica_ori.txt", "a")) == NULL) {
		fprintf(stderr, "Can not open file\n");
	}
...
//
	...
	sprintf(output, "OUTPUT\\PCA_BASIS.bmp"); //PCA基底bmpで出力
	for (i = 0; i < 64; i++)
		for (j = 0; j < 64; j++)
	...
	for (j = 1; j < 5; j++)
		for (i = 0; i < 64; i++) {
			...
			sprintf(output, "OUTPUT\\test\\%d\\%d\\BASIS.bmp", j, 8 * m + n);
	...
	sprintf(output, "OUTPUT\\ICA_SAI.bmp"); //再構成画像bmpで出力
	for (i = 0; i < 256; i++)
		for (j = 0; j < 256; j++)
