	...
	if (yn == 'n') {
		...
		// 
			...
			if (yn == 'y') {
						...
						for (j = 0; j < 1024; j++) {
							if (no_op[j] == 1) {
								...
								if (dct_mse[j] > comb2[j][a][b][0]) {
									comb_result2[a][b][0] += comb2[j][a][b][0] - dct_mse[j];//dctからの画質改善量を累積
				...
				for (a = 0; a < 10; a++) {
					comb_after_sort[a][2] = 99;
					comb_after_sort[a][3] = 99;
					comb_after_sort[a][4] = 99;
				}
				...
				for (j = 0; j < 1024; j++) {
					if (c != 99) {//選出基底3津の時
						...
						if (ica_basis2[64][j] == 2 && comb2[j][a][b][0] < dct_mse[j] && a != 99) {
							ny[a][j] = y[a][j];
							ny[b][j] = y[b][j];
							no_op[j] = 1;
						}
