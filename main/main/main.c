...
//読み込むファイル名
	...
	if (yn == 'n') {
		...
		//b_entropy_ica(ny);
			...
			fprintf(fp8, ",=(C%d+D%d+F%d)", excel_temp, excel_temp, excel_temp);
			fprintf(fp8, ",=(C%d+D%d+F%d+$B$2*G%d)", excel_temp, excel_temp, excel_temp, excel_temp);
			psnr_temp2 = psnr(origin, ica_sai);
			fprintf(fp8, ",%lf", psnr_temp2);
			fprintf(fp8, ",,=(B%d-J%d)", excel_temp, excel_temp);
			fprintf(fp8, ",,=(N%d/$B$2)", excel_temp);
			excel_temp++;
			// / ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			if (yn == 'n') {
				...
				for (a = 0; a < 64; a++) {
					...
					for (j = 0; j < 1024; j++) {
						...
						if (ica_basis2[64][j] != 66) {//基底1個領域を対象
				...
				for (a = 0; a < 64 - 1; a++) {
					for (b = a + 1; b < 64; b++) {
						...
						for (j = 0; j < 1024; j++) {
							...
							if (ica_basis2[64][j] != 66) {//基底2個領域を対象
					...
					for (b = a + 1; b < 64 - 1; b++) {
						for (c = b + 1; c < 64; c++) {
							for (j = 0; j < 1024; j++) {
								...
								//1vs2vs3個で画質比較
								if (comb2[j][k][l][0] < comb[j][m][0] && dct_mse[j] > comb2[j][k][l][0]) {
					...
					for (b = a + 1; b < 64; b++) {
						for (j = 0; j < 1024; j++) {
							...
							if (comb[j][a][0] > comb[j][b][0])
								...
								//1vs2個の画質比較
							if (comb2[j][a][b][0] < comb[j][m][0] && dct_mse[j] > comb2[j][a][b][0] && comb2[j][a][b][1] > -0.00004) {
							...
							if (dct_mse[j] > comb[j][k][0] && comb[j][k][1] > -0.00004) {
				...
				for (a = 0; a < 64; a++)
					for (j = 0; j < 1024; j++)
						if (dct_mse[j] > comb[j][a][0] && comb[j][a][1] > -0.00004) {
				...
				if (Q==70||Q==10) {
					a = (int)comb_after_sort[20][2];
					b = (int)comb_after_sort[20][3];
					c = (int)comb_after_sort[20][4];
				}
				else{
					a = (int)comb_after_sort[10][2];
					b = (int)comb_after_sort[10][3];
					c = (int)comb_after_sort[10][4];
				}
				...
				for (j = 0; j < 1024; j++) {
					if (a != 99 && b != 99 && c != 99) {//選出基底3津の時
						...
						else if (dct_mse[j] > comb[j][m][0] && comb[j][m][1] > -0.00005) {
							ny[m][j] = y[m][j];
							no_op[j] = 1;
						}
					}
						...
						//1vs2個の画質比較
						if (comb2[j][a][b][0] < comb[j][k][0] && dct_mse[j] > comb2[j][a][b][0] && comb2[j][a][b][1] > -0.00004) {
							ny[a][j] = y[a][j];
							ny[b][j] = y[b][j];
							no_op[j] = 1;
						}
						else if (dct_mse[j] > comb[j][k][0] && comb[j][k][1] > -0.00004) {
							ny[k][j] = y[k][j];
							no_op[j] = 1;
						}
					}
					else if (a != 99 && b == 99 && c == 99) {
						if (comb[j][a][0] < dct_mse[j] && comb[j][a][1] > -0.00004) {
							ny[a][j] = y[a][j];
							no_op[j] = 1;
						}
					}
